...
import { playwrightStringifyChromeRecording } from '../main.js'
...
export async function runTransforms({
  ...
  return files.map(async (file) => {
    console.log(chalk.green(`Running Playwright Chrome Recorder on ${file}\n`))

    const recordingContent = readFileSync(`${file}`, 'utf8')
    const stringifiedFile = await playwrightStringifyChromeRecording(
      recordingContent
    )
